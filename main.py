...
args_navbar = {
    "tipo": "navbar2",
    "opcoes": ["Resumo",'Introdução', "Fundamentos de ML", "Redes Neurais", "Aplicações"],
}
...
if st.session_state.page == "Redes Neurais":
    st.markdown("## Redes Neurais Artificiais", unsafe_allow_html=True)
    content = conteudo.redes_neurais
    cols = st.columns([0.15,0.85])
    for cnt in content:
        atalho = cnt['titulo'].replace(" ", "_").lower()
        st.sidebar.markdown(f"[{cnt['titulo']}](#{atalho})",unsafe_allow_html=True)
